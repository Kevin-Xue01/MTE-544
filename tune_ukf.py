...
# Define base folder using the training iteration.
...
def load_dataset(folder):
    # Build file paths using the localization mode name.
...
def optimize_noise_params(datasets):
    # Initial guess for the 6 process noise and 4 measurement noise parameters.
    Q = [1.03218359e-01, 1.01333363e-01, 1.00000000e-06, 1.08341521e-01, 1.05329996e-01, 1.01631006e-01]
    R = [1.08078889e-01, 1.00000000e-06, 7.82164814e-02, 7.96654837e-02]
    initial_guess = Q + R
    bounds = [(1e-6, None)] * 10
...
def main():
    # List of folder names (update these names as needed).
    ...
    # Optimize the noise parameters using the datasets.
    ...
    # Iterate through all subfolders in the base folder for detailed plotting.
    for folder in os.listdir(base_folder):
        folder_path = os.path.join(base_folder, folder)
        if os.path.isdir(folder_path):
            # Define file paths using the localization mode name.
            ...
            if os.path.exists(ground_truth_file) and os.path.exists(odom_file) and os.path.exists(imu_file) and os.path.exists(original_estimate_file):
                ...
                # Use the optimal noise parameters from tuning.
                optimal_q_diag = result.x[:6]
                optimal_r_diag = result.x[6:]
                estimates = run_ukf(optimal_q_diag, optimal_r_diag, odom, imu, ground_truth)
                
                # Compute MSEs for ground truth versus different estimates.
                ...
                plt.plot(original_estimate[:, 0], original_estimate[:, 1], 
                         label=f'Original {localization_mode.name} (MSE={mse_gt_original:.4f})', 
                         color='blue', alpha=0.7, linestyle=':')
                plt.plot(estimates[:, 0], estimates[:, 1], 
                         label=f'Trained {localization_mode.name} (MSE={mse_gt_est:.4f})', 
